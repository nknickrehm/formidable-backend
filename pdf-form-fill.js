...
function updateOptionButtonValue (handles, fieldDictionary, value) {
  ...
  if (isWidget || !fieldDictionary.exists('Kids')) {
    ...
    if (value === null) {
      ...
      const apDictionary = handles
        .reader
        .queryDictionaryObject(fieldDictionary, 'AP')
        .toPDFDictionary();
      const nAppearances = handles
        .reader
    ...
    if (value === null) {
      ...
      // grab the non off value. that should be the yes one
      const widgetDictionary = handles
        .reader
    ...
    for (let i = 0; i < fieldsReferences.length; i += 1) {
...
function writeAppearanceXObjectForText (handles,
  formId,
  fieldsDictionary,
  text,
  inheritedProperties
  ...
  // would have been better to just refer to it
  ...
  // Will use Tj with "code" encoding to write the text,
  // assuming encoding should work (??). if it won't i need real fonts here
  ...
  xobjectForm.getContentContext()
    .writeText(text, 0, 4, {font: handles.font, size: 12, color: 0x00, colorspace: 'gray'});
...
function updateTextValue (handles, fieldDictionary, _value, isRich, inheritedProperties) {
  let value = _value;
...
function updateChoiceValue (handles, fieldDictionary, value, inheritedProperties) {
  ...
  // start with value, setting per one or multiple selection.
  // also choose the text to write in appearance
  let textToWrite;
  if (typeof (value) === 'string') {
    // one option
    modifiedDict
    ...
    value.forEach((singleValue) => {
      const bytesArray = new hummus.PDFTextString(singleValue).toBytesArray();
      handles.objectsContext.writeLiteralString(bytesArray);
    });
...
function updateFieldWithValue (handles, fieldDictionary, value, inheritedProperties) {
  ...
  // This must be a terminal field. meaning it is a field, and it either has no kids,
  // it also holding Widget data or that it has one or more kids defining its widget annotation(s).
  // Normally it would be One but in the case of a radio button, where there's one per option.
...
function writeFieldAndKids (handles, fieldDictionary, inheritedProperties, baseFieldName) {
  ...
  // so write the dictionary as is, dropping kids.
 ...
 * assuming that's in indirect object and having to write the dict,finish the dict,
 * indirect object and write the kids
 */
...
function writeFilledForm (handles, acroformDict) {
  ...
  if (fields) {
    ...
    writeFilledFields(handles, modifiedAcroFormDict, fields, {}, '');
    // will also take care of finishing the dictionary and indirect object,
    // so no need to finish after
  } else {
    handles
...
function fillForm (writer, data, callback) {
  // setup parser
  ...
  const font = writer.getFontForFile(path.join(__dirname, 'Roboto-Regular.ttf'));
  // lets put all the basics in a nice "handles" package,
  // so we don't have to pass each of them all the time
  const handles = {
    writer,
    reader,
    copyingContext,
